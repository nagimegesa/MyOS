...
## Day09
1. 检查内存大小
   * 在编写内存管理程序之前, 需要知道内存的大小, 这里作者决定的内存大小是32M
   * 检查内存大小的基本方法就是暴力的先每一个地址写入和读取数据, 若读取和写入的数据相同则认为内存有效, 否则内存不可用。
   * 由于一些特殊原因, 作者使用的是汇编编写的函数一次读写内存
2. 编写内存管理程序
   * 操作系统需要管理内存, 管理内存的方式有很多种, 其核心思想是通过一定的数据结构, 记录内存的使用情况 **(在编写操作系统时操作系统本身使用的内存位置一般都是手动指定。以自身为例，在MyOS.s(操作系统的汇编部分)就已经初始化了操作系统所需的堆栈, 这一部分是作者提供的, 看不懂（￣︶￣）↗　, 并且在bootpack.c(操作系统的C语言部分)中也有相当一部分是手动指定使用的内存地址, 比如初始化GPT、IDT和PIC,详见上面)**,于是运行在系统上的用户软件只能通过调用操作系统提供的API返回可以使用的内存地址 **(C语言的malloc底层也是在调用本地系统的API, 只是不同操作系统对应的分配算法和管理方式可能不同)**, 随意指定内存位置则被操作系统拦截引发中断进而产生错误导致程序奔溃(指针越界, 蓝屏啦之类的), 这里可以结合虚拟内存机制一起理解 
   * **关于堆栈的一些理解**
     * 程序运行时, 由操作系统为程序分配堆栈, **此时栈的大小已经固定, 并被操作系统记录**(据说windows的栈大小默认是1M), 一开始, 操作系统会为程序分配一定大小的堆, 在堆的大小不足时, 再由操作系统分配。结合虚拟内存, 可以知道一个程序只有一个栈, 可以有多个堆, 栈的变化在程序被编译的时候就已经确定 **(注意这里栈的大小是固定的, 变化的只是栈指针)**, 只有堆的大小是动态的。
   * 关于管理内存使用的数据结构
      * 在书中作者介绍了两种管理方式, 第一种是 **将内存分页, 暴力的使用一个数组记录每一页的使用情况(bitmap也可以)**。这种方式缺点是自身使用的内存比较多(将内存分成4KB每页, 如果不采用bitmap, 就需要占用0.02%的内存大小 ps::好像也不多), 优点是管理简单。 第二种也是书中使用的方式:**用一个较小的一维数组, 数组中的每个元素记录内存的起始地址和从这个地址开始可使用的内存大小**, 这种方式优点是自身占用的内存较少, 缺点是管理比较复杂。
      * 实际上我们常使用的windows和linux的内存管理:windows使用的貌似跟接近于作者介绍的第一种(雾), 而linux使用的则接近第二种, 不同的是linux采用的基本数据结构是链表