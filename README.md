...
2. 编写叠加处理程序
   * 在这里由于之前的思路和作者的不太一样, 导致代码变动很大, 以下是主要区别
     1. 增加了**struct SheetManger**结构体, 作为不同层的管理者
     2. 增加了**struct Sheet**结构体, 储存某一层的具体信息、像素分布。 
     3. graphic.c(h) 作为绘图模块由原先的直接在显卡中写入数据, 修改为了根据**struct Sheet**中储存的数据写入显卡
     4. 由3, 将mouse和desktop独立出来成为一个Sheet, drawRect改为向Sheet中写入数据
     5. 绘图由原先的单独绘制，改为由**SheetRefresh**函数统一集中绘制，由此删除了drawDesktop和drawMouse
     6. 由5, 移动鼠标由原先的单独移动, 改为使用**sheetMove**函数统一处理Sheet的移动
   * 优化处理
     * 每次变化重新绘制所有层效率过于低下, 因此采用重新绘制部分的方式
     * 部分绘制的基本原理就是每次由变化传入要重新绘制的矩形部分的坐标以及宽高, 然后与其他图层检测是否重叠, 重新绘制重叠部分即可 **(sheetRefreshSub函数)**
     * **检测重叠要使用优化的方法(ps::这里没太懂作者写的, 所以按照自己的想法来写的)**。假设矩形1(2)的左上角和右上角坐标分别为(x1, y1)(x2,y2)(x3, y3)(x4,y4)。假设重叠矩形左上和右下坐标分别为(xs,ys)(xx,yx)则有:
      ~~~c 
      xs = max(x1, x3)
      ys = max(y1, y3)
      xx = min(x2, y2)
      ...
      xs > xx || yx > ys
      ~~~
     * 使用检测重叠后, 效果比较好, 但是在多层窗口的情况下依旧有较为明显的闪烁。因此增加一个 **sheetMap** 记录屏幕上每一个显示点对应的图层, 每次绘制按sheetMap 只重新绘制最上方图层, 最终闪烁消失。
   * 增加窗口
     * 新增加window.c(h) 存放窗口相关的函数   