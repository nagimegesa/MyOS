...
- [目录](#目录)
  ...
  - [Day12 & Day13](#day12--day13)
  - [Day14](#day14)
  - [Day15 & Day16](#day15--day16)
  - [Day17](#day17)
  - [Day18 & day19](#day18--day19)
...
## Day12 & Day13
1. 设置定时器
   * 主板上自带PIT模块用以定时, 只需像鼠标键盘那样设置好中断就可以使用 **(具体内容见timerInterrupt)**
   * 在这里作者设置为每秒产生100个中断
2. 优化定时器
   * 在中断来时处理中断,比较卡顿, 因此作者使用了用数组存链表的方式优化, 而源码直接使用了链表(*^_^*), 该链表需要提供比较函数以用于排序 **(这个链表为达到可复用性所以全部由宏定义, 实现了类似C++函数模板的功能(╬▔皿▔)╯)**
   * 同时作者将鼠标键盘以及定时器的中断消息放到同一个消息队列集中处理, 加快整体性能。
   * 在这里, 只是将鼠标和键盘的消息放到一个队列集中处理(╬▔皿▔)╯, 定时器采用的回调函数的方式, 超时之后调用回调函数,并在链表中删除该定时器, 但是在超时一瞬间有明显卡顿现象。留给后续处理(*^_^*)。
3. 关于用宏实现模板效果的方法
   * 使用宏连接的功能把类型名和函数名连接起来, 为每一个类型单独生成代码, 再使用宏编写统一的接口调用相应的函数。
## Day14
1. 提高分辨率
   * 作者在这里修改了分辨率, 其基本原理就是修改BIOS的显卡模式 **(Day04)**, 现在的分辨率为1026*760
   * 增加分辨率之后, 系统速度变快了(●'◡'●), 原先定时器超时鼠标闪烁现象消失
2. 处理键盘中断
   * 键盘中断输处的信息对应键盘上的字符, 用哈希表处理即可
   ~~~c
   // 作者提供的表
   ...
   ~~~
3. 增加窗口的移动功能
   * 在处理鼠标中断的时候, 检测鼠标点击以及sheetMap, 若sheetMap对应sheet不为桌面且鼠标位置在标题栏, 移动sheetMap对应sheet即可
4. 增加输入光标闪烁功能
   * 作者在这里只有一个窗口的情况下,简单的增加了一个闪烁功能(个人认为这里不具备可扩展性)。但是由于自己的定时器采用回调函数的方式, 无法在调用函数时变化参数, 因此没有实现这个功能, 目前也没有想到较好的实现方式, 留给日后解决。

## Day15 & Day16
1. 实现多任务
   * 多任务就是实现多道操作系统, 基本思想就是CPU快速切换执行不同的任务, 以至于好像就是在同时执行任务, 在这里作者设定的切换时间为0.2秒, 即每0.2秒切换一次任务
   * 切换任务时, 需要保存当前任务的相关信息, 然后产生一个特殊的中断, 改中断指出下一个任务的地址, 然后CPU读取相关信息, 开始执行
   * 在这里使用定时器设置0.2秒的定时, 每到0.2秒就产生中断
2. 设置优先级
   * 多任务需要设置不同的优先级, 作者在这里使用定时器时间作为优先级, 即任务设置的CUP时间越短, 优先级越高。
   * 采用这种方式还需要设定不同的优先层级, 即每一层对应不同的优先级, 上层优先级发生呢中断优先处理。
   * 在这里作者采用在**TaskManger**中加一层间接性的方式, 用TaskManger管理不同的层级, 而使用**TaskLevel**管理不同的任务
3. **memory.c中memoryFree函数有问题** 已经更改, 介于特殊原因(╬▔皿▔)╯, 之前的文件就不修改了！！

## Day17
1. 制作命令行窗口
   * 命令行窗口就是命令行窗口啦, 在这里根据作者使用的方法,使用一个单独的任添加了一个窗口作为命令行窗口
   * 光标闪烁使用作者介绍的方式/(ㄒoㄒ)/~~, 即使用定时器器在一个死循环里检测定时器缓冲区是否有数据, 然后根据数据判断是哪一个定时器, 在这里定时器设置的时间间隔为0.5秒
2. 处理键盘输入
   * 作者使用的键盘输入模式为第一个任务检测到键盘数据之后处理完毕再发送给第二个任务, 其原理是使用一个共享的地址内存
   * 在这里, 个人认为为了降低耦合性, 提高可读性, 采用了一个**WindowManger**统一记录所有窗口的类型以及窗口对应的权重, 记录的数据结构为链表, 由权重从高到低的顺序。当有键盘中断消息时,检查最顶层窗口是不是可写的 **(命令行窗口)**, 如果是则向改窗口发送要写的数据
   * 同时在这里, 并没有处理像**tab**、**enter**以及**ESC** 上下左右等这样的特殊数据, 只对shift以及caps lock等大小相关信息做了区分
   * 编写的键码基于作者提供的表改写而成 **(可能是作者那里的键盘和我们这里的不太一样(*^_^*))**
   ~~~c
   // 键盘中断产生的消息对应的表
   // 上面的为小写, 下面的为大写, 产生键盘中断时按下去产生小写的消息, 松手产生大写的消息
   // 这里只对消息的消息处理, 判断大小写是基于shift和caps lock
    ...
    static char keytable1[60] = {
        ...
        'B', 'N', 'M', '<', '>', '?', 0,   '*', 0,   ' ', 0};
   ~~~
   * **作者的使用的表的大小有128个, 用以处理一些特殊的字符, 在这里为了节省不必要的麻烦就没有处理。并且个人的电脑上没有部分特殊的按键（︶^︶）。注意这里和前两条说明不同, tab以及enter等按键是留到以后处理**
   * **在使用长按shift加字母之后, 即使依旧按着shift也检测不到,要松手才能重新被检测**
## Day18 & day19
1. 制作mem命令
   * mem 命令显示剩余的内存空间**这里发现内存越来越少,怀疑定时器的内存释放可能有问题**
2. 制作cls命令
   * cls 命令简单的清屏
3. 制作ll命令
   * ll 命令当前目录文件相关信息
   * ll 需要读取文件的相关信息, 文件的信息再系统启动的时候已经内读取到内存里(**IPL.s在启动时读取的11个扇区**),文件的存储格式由微软定义, 在这里只是按照格式将文件信息读取出来。
4. 制作cat 命令
   * cat 命令读取文件内容
   * ll 命令读取的文件夹信息格式中记录了文件所在的位置
   * 文件内容的存储格式是FAT **(数组实现的链表, 每个节点大小512字节)**, 并且由window的算法压缩
   * 因此解压缩之后(**具体算法见函数initFATInfo**)就可以直接读取文件内容