...
void keyBoardInterrupt(int* esp) {
    FIFOBuf* buf = getMsgBuf();
    writePort8(PIC0_OCW2, 0x61);
    FIFOPush(buf, readPort8(PORT_KEYDAT) + MSG_KEYBOARD_MIN);
}
...
void mouseInterrupt(int* esp) {
    ...
    FIFOPush(buf, readPort8(PORT_KEYDAT) + MSG_MOUSE_MIN);
}

void timerInterrupt(int* esp) {
    writePort8(PIC0_OCW2, 0x60);
    TimerManger* manger = getTimerManger();
    ++manger->time;
    if (manger->time < manger->nextTime) return;
    timeOut();
}
